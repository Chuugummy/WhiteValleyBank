var g_posts = [];

function renderNews() {
    g_posts = [];
    
    axios.post(
        "https://brixybot.bubbleapps.io/version-test/api/1.1/wf/bank_snews"  
    ).then(response => {
        var posts = response.data.response.posts;
        var append = [];
        
        posts.forEach(post => {
            g_posts.push(post);
            
            var data = `
                <a onclick="seeNews('${post._id}')"href="#"><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 16 16" fill="currentColor" class="bi bi-newspaper" style="margin-right: 8px;margin-bottom: 3px;">
                <path fill-rule="evenodd" d="M0 2.5A1.5 1.5 0 0 1 1.5 1h11A1.5 1.5 0 0 1 14 2.5v10.528c0 .3-.05.654-.238.972h.738a.5.5 0 0 0 .5-.5v-9a.5.5 0 0 1 1 0v9a1.5 1.5 0 0 1-1.5 1.5H1.497A1.497 1.497 0 0 1 0 13.5v-11zM12 14c.37 0 .654-.211.853-.441.092-.106.147-.279.147-.531V2.5a.5.5 0 0 0-.5-.5h-11a.5.5 0 0 0-.5.5v11c0 .278.223.5.497.5H12z"></path>
                <path d="M2 3h10v2H2V3zm0 3h4v3H2V6zm0 4h4v1H2v-1zm0 2h4v1H2v-1zm5-6h2v1H7V6zm3 0h2v1h-2V6zM7 8h2v1H7V8zm3 0h2v1h-2V8zm-3   2h2v1H7v-1zm3 0h2v1h-2v-1zm-3 2h2v1H7v-1zm3 0h2v1h-2v-1z"></path>
                </svg>${post.title}<br /></a>
            `;
            
            append.push(data);
        });
        
        document.getElementById("news-dom").innerHTML = append.join("\n");
        readParams();
    });
}

function seeNews(id) {
    var post = g_posts.find((post) => post._id == id);
    
    document.getElementById("newsRenderDom").innerHTML = marked(post.body);
    
    document.getElementById("newsRenderTitle").innerHTML = post.title;
    
    $("#readNews").modal("show");
}

function readParams() {
    const urlParams = new URLSearchParams(window.location.search);
    if (urlParams.has("viewNews")) {
        var xid = urlParams.get("viewNews").toString();

        var post = g_posts.find((post) => post._id == xid);
        seeNews(post._id);
    }
}